...
def list():
    ...
    return send_from_directory(app.static_folder, "list.json", cache_timeout=0)
...
def announce():
    ...
    if "%s/%d" % (server["ip"], server["port"]) in app.config["BANNED_SERVERS"]:
        ...
        "address" in server
        and "%s/%d" % (server["address"].lower(), server["port"])
        in app.config["BANNED_SERVERS"]
    ):
        return "Banned (Server).", 403
    elif (
        "address" in server
        and server["address"].lower() in app.config["BANNED_SERVERS"]
    ):
        return "Banned (Server).", 403
    ...
    server["clients_top"] = (
        max(server["clients"], old["clients_top"]) if old else server["clients"]
    )
...
def finishRequestAsync(server):
    th = Thread(name="ServerListThread", target=asyncFinishThread, args=(server,))
    th.start()
...
def asyncFinishThread(server):
    ...
    if checkAddress:
        ...
        if not server["ip"] in addresses:
            app.logger.warning(
                "Invalid IP %s for address %s (address valid for %s)."
    ...
    if not server["ping"]:
        app.logger.warning(
            "Server %s:%d has no ping." % (server["address"], server["port"])
        )
        return
    ...
    def purgeOld(self):
        with self.lock:
            self.list = [
                ...
                if time.time() <= server["update_time"] + app.config["PURGE_TIME"]
            ]
            self.save()
                ...
                json.dump(
                    {
                        ...
                        "total_max": {
                            "servers": self.maxServers,
                            "clients": self.maxClients,
                        },
                        "list": self.list,
                    },
                    fd,
